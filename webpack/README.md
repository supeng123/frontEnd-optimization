...
- [Webpack-common-question](#webpack-common-question)
## Modulization
...
~~~
## Webpack-common-question
~~~
what's the differerce between webpack and gulp or grunt
Webpack is to generate module bundle, what separates webpack from
other tools is that it supports code-spliding,modulization, global analytics.

what is bundle, chunk, module
bundle is output file generated by webpack, chunk is the splitted
code blocks after webpack has done the modulization analytics, module is the single area in developement

what is loader and plugin
loader is designed for dealing with all different types of files, and import all of them into the bunldes, plugin is used to define the way how
webpack generate the bundle, it executing during the whole webpack lifecycle

what is the difference between webpack-dev-server and http server
webpack-dev-server use the memory to save the bundles in the developement environment, it supports module hot replacement, and is more
efficient than traditional http server.

what's Tree-shaking, dose CSS has Tree-shaking
Tree-shaking means remove the redudant codes which has been imported into the project but never actually used. webpack uses
uglifyJSPlugin to do the Tree-shaking, meanwhile, webpack can also use purifyCSS to Tree-shaking CSS.
